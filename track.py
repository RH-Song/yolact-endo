...
def evalimages(net: Yolact, input_folder: str, output_folder: str):
    ...
    print()
    for p in sorted(Path(input_folder).glob('*')):  # Guoslu
        path = str(p)
...
def get_prediction(net: Yolact, path: str):
    frame = torch.from_numpy(cv2.imread(path)).cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)
    ...
    with timer.env('Postprocess'):
        save = cfg.rescore_bbox
        cfg.rescore_bbox = True
        classes, scores, boxes, masks = postprocess(preds, w, h, visualize_lincomb = args.display_lincomb,
                                        crop_masks        = args.crop,
                                        score_threshold   = args.score_threshold)
        cfg.rescore_bbox = save
        # print("t:-----------------------------")
        # print(t)
    return classes, scores, boxes, masks, h, w


def get_predictions(net: Yolact, input_folder: str):
    for p in sorted(Path(input_folder).glob('*')):
        path = str(p)
        print("path =  " + path)
        classes, scores, boxes, masks, h, w = get_prediction(net, path)
        masks = masks.view(-1, h * w).cuda()
        boxes = boxes.cuda()
        print("masks = ")
        print(masks)
        print("boxes = ")
        print(boxes)
        mask_iou_cache = _mask_iou(masks, masks)
        bbox_iou_cache = _bbox_iou(boxes.float(), boxes.float())
        print("mask_iou_cache = " )
        print(mask_iou_cache)
...
def save_track_img(img_path, scores, boxes, masks, trajectory_id, save_path, h, w, fps_str='', undo_transform=True):
    ...
    num_dets_to_consider = boxes.shape[0]
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        mask_alpha = 0.45
        colors = torch.cat(
            [get_color(0, on_gpu=img_gpu.device.index).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
        masks_color = masks.repeat(1, 1, 1, 3) * colors * mask_alpha
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                # _class = cfg.dataset.class_names[classes[j]]
                text_str = '%s %d: %.2f' % ('tool', trajectory_id[j], score) if args.display_scores \
                    else ('%s %d' % ('tool', trajectory_id[j]))
...
def trackor(net: Yolact):
    net.detect.use_fast_nms = args.fast_nms
    net.detect.use_cross_class_nms = args.cross_class_nms
    cfg.mask_proto_debug = args.mask_proto_debug
    if args.images is None:
        print("args.images can not be none!")
        return
    # image data folder
    input_folder, output_folder = args.images.split(':')
    tracks_active = []
    tracks_finished = []
    tracks_number = 0
    for p in sorted(Path(input_folder).glob('*')):
        ...
        for track in tracks_active:
            if classes.size(0) > 0:
                ...
                track_last_box = track['boxes'][-1].unsqueeze(0)   # tensor([[ 468,  613, 1033, 1063]])
                track_last_mask = track['masks'][-1].unsqueeze(0)
                # print("track_last_box = ")
                # print(track_last_mask)
                # print(masks)
                mask_iou_cache = _mask_iou(track_last_mask, masks)  # tensor([[0.3979, 0.0000, 0.0000]], device='cpu')
                best_match_idx = torch.argmax(mask_iou_cache, dim=1)[0]     # tensor(0, device='cpu')
                best_match_iou = mask_iou_cache[0][best_match_idx]      # tensor(0.3979, device='cpu')

                if best_match_iou > 0.5:
                    track['boxes'].append(boxes[best_match_idx])
                    track['masks'].append(masks[best_match_idx])
                    track['scores'].append(scores[best_match_idx])
                    track['frameID'].append(frame_id)
                    # remove best matching detection from detections
                    boxes = boxes[torch.arange(boxes.size(0)) != best_match_idx]
                    masks = masks[torch.arange(masks.size(0)) != best_match_idx]
                    scores = scores[torch.arange(scores.size(0)) != best_match_idx]
                    classes = classes[torch.arange(classes.size(0)) != best_match_idx]

                    updated_tracks.append(track)
            # if track was not updated
            if len(updated_tracks) == 0 or track is not updated_tracks[-1]:
                # if track was not updated in 3 frames, not finished
                if 3 > int(frame_id) - int(track['frameID'][-1]):
                    updated_tracks.append(track)
                # if length > 5, finish, else throw away
        ...
        for i in range(boxes.shape[0]):
            new_tracks.append({'boxes': [boxes[i]],     # {'boxes': [tensor([ 468,  613, 1033, 1063])], ……
                               'masks': [masks[i]],
                               'scores': [scores[i]],   # [tensor(1.)]
                               'frameID': [frame_id]})  # ['2103']
        # # print("new tracks = ")
        # print(new_tracks[0]['boxes'])
        # print(new_tracks[0]['scores'])
        # print(new_tracks[0]['frameID'])
        tracks_active = new_tracks + updated_tracks
    ...
    for p in sorted(Path(input_folder).glob('*')):
        path = str(p)
        frame_id = path[-8:-4]
        scores_, boxes_, masks_, trajectory_id = [], [], [], []

        for i in range(len(tracks_finished)):
            track = tracks_finished[i]
            if track['frameID'].count(frame_id):
                idx = track['frameID'].index(frame_id)
            else:
                continue
            # if trajectory_id == []:
            #     scores_ = track['scores'][idx]
            #     boxes_ = track['boxes'][idx]
            #     masks_ = track['masks'][idx]
            # else:
            #     scores_ = torch.cat((scores_, track['scores'][idx]), 0)
            #     boxes_ = torch.cat((boxes_, track['boxes'][idx]), 0)
            #     masks_ = torch.cat((masks_, track['masks'][idx]), 0)
            scores_.append(track['scores'][idx].cpu().numpy().tolist())
            boxes_.append(track['boxes'][idx].cpu().numpy().tolist())
            masks_.append(track['masks'][idx].cpu().numpy().reshape(h,w).tolist())
            trajectory_id.append(i)

        if len(trajectory_id)>0:
            print(boxes_)
            print(scores_)
            print(torch.Tensor(masks_))
            print(torch.Tensor(masks_).shape)
            save_folder = output_folder + '/' + frame_id + '.png'
            save_track_img(path, scores_, boxes_, masks_, trajectory_id, save_folder, h, w, undo_transform=False)
...
if __name__ == '__main__':
    ...
    if args.trained_model == 'interrupt':
        print("interrupt train model……")
        args.trained_model = SavePath.get_interrupt('weights/')
    elif args.trained_model == 'latest':
        print("latest train model……")
